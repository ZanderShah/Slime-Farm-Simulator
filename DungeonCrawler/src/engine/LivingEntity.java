...
public abstract class LivingEntity implements Drawable, Serializable {
	...
	public void drawEntityDetails(Graphics g, Vector2D offset) {
		...
		g.fillRect((int) shifted.getX() - getWidth() / 2, (int) shifted.getY() - getHeight() / 2 - 10,
				(int) (getStats().getHealth() / getStats().getMaxHealth() * getWidth()), 5);
	}
		...
		for (int i = 0; i < effects.size(); i++)
			if (effects.get(i).getType() == StatusEffect.HEALTH)
				if (effects.get(i).getAbsStrength() > 0)
					g.drawImage(SpriteSheet.HEALING, (int) shifted.getX() - SpriteSheet.HEALING.getWidth(null) / 2,
							(int) shifted.getY() - SpriteSheet.HEALING.getHeight(null) / 2, null);
				else
					g.drawImage(SpriteSheet.POISON, (int) shifted.getX() - SpriteSheet.POISON.getWidth(null) / 2,
							(int) shifted.getY() - SpriteSheet.POISON.getHeight(null) / 2, null);
			else if (effects.get(i).getType() == StatusEffect.RAGE)
				g.drawImage(SpriteSheet.RAGE, (int) shifted.getX() - SpriteSheet.RAGE.getWidth(null) / 2,
						(int) shifted.getY() - SpriteSheet.RAGE.getHeight(null) / 2, null);
	}
}