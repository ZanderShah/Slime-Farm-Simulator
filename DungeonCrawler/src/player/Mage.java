...
public class Mage extends Player
{
	...
	public void draw(Graphics g, Vector2D offset)
	{
		...
		g.drawImage(SpriteSheet.MAGE_IMAGES[0], (int) shifted.getX()
		...
		if (beaming > 0)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				r.addDamageSource(new BeamParticle(60, getPos().add(spray),
						beamDirection.multiply(10.0), true));
			}
		}
		super.update(cs, r);
	...
	public void ability1(Point p, Room r)
	{
		if (getAttacking() == 0 && getCooldown(1) == 0)
		{
			setAttacking(120);
			beaming = 120;
			beamDirection = (new Vector2D(p)).subtract(Test.middle)
					.getNormalized();
		}
	}
	...
	public void ability2(Point p, Room r)
	{
		if (getCooldown(2) == 0)
		{
			...
			setCooldown(2, Constants.MAGE_AB2_COOLDOWN
					+ Constants.MAGE_DEBUFF_LENGTH);
	...
	public void ability3(Point p, Room r)
	{
		if (getCooldown(3) == 0)
		{
			r.addDamageSource(new FireCircle((new Vector2D(p)).add(getPos())
					.subtract(Test.middle), 100, 30, 300, true, 3));
