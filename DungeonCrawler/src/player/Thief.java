...
public class Thief extends Player {
	...
	public void draw(Graphics g, Vector2D offset) {
		...
		g.drawImage(SpriteSheet.THIEF_IMAGES[getDirection()][(getAbilityActive(1) == 0 ? 0 : 1)], (int) shifted.getX() - getWidth() / 2, (int) shifted.getY()
				- getHeight() / 2, null);
	}
	...
	public boolean attack(Point p, Room r) {
		...
		if (attacked) {
			r.addDamageSource(new SwordDamageSource(getPos(), Constants.THIEF_SWORD_SIZE, (int) getAttackDir().getAngle() - Constants.THIEF_SWING_ANGLE / 2,
					Constants.THIEF_SWING_ANGLE, getStats().getAttackTime(), true, (getAbilityActive(1) != 0 ? Constants.THIEF_DAMAGE * 3
							: Constants.THIEF_DAMAGE), Constants.THIEF_KNOCKBACK, getID(), 1), getStats().getDamageMultiplier());
	...
	public void ability3(Point p, Room r) {
		if (getCooldown(3) == 0) {
			...
			r.addDamageSource(new Knife(getPos(), new Vector2D(knifeDir.getAngle() - Constants.KNIFE_SPREAD), true, damage, getID()), getStats()
					.getDamageMultiplier());
			r.addDamageSource(new Knife(getPos(), knifeDir, true, damage, getID()), getStats().getDamageMultiplier());
			r.addDamageSource(new Knife(getPos(), new Vector2D(knifeDir.getAngle() + Constants.KNIFE_SPREAD), true, damage, getID()), getStats()
					.getDamageMultiplier());
